...
<head>
	...
	<style>
		.errormessage, .errormessage a { background-color:#493838; border:none; color:#FFFFFF; }

	</style>
	<script type="text/javascript" charset="utf-8" src="/js/formFunctions.js"></script>
    <?php
			if (isset($this->ErrorMessage1) and $this->ErrorMessage1 == 'yes') {
	?>
	<script>
		...
		window.onload = function() {
		 Shadowbox.open({
							modal:      true,
					        content:    '<div class="errormessage" style="width:328px;" ><p class="error" style="width:325px;" >This email id already exists on Josiemarancosmetics.com. Would you like to:<ul style="list-style:decimal outside none; padding-left:20px;"><li><a href="/signin" >Log in and check out as a member</a></li><li><a href="javascript:Shadowbox.close()">Continue with guest checkout using a different email</a></li></ul></p></div>',
					        player:     "html",
					        title:      "",
					        height:     121,
					        width:      360
...
<body id="checkout">
	...
	<div id="main">
		...
		<form action="<?php echo secure_base_url() ?>guestcheckout/submit" method="post" accept-charset="utf-8" onSubmit="return isValidCreditCard();" >
		...
		<div class="accountinputbox">
			...
			<fieldset id="account_information">
				...
				<p>
					...
					<label for="phone">Phone* (shipper requires this)<?php echo form_error('phone', '<span class="error">&nbsp;&nbsp;', '</span>'); ?></label><br>
				 ...
				 <p>If you would like to be able to log back in and check your order status, please configure a password here</p> 

				 <p>

						<label for="password">Password<?php echo form_error('password', '<span class="error">&nbsp;&nbsp;', '</span>'); ?></label><br>

						<input type="password" value="<?php echo set_value('password') ?>" id="password" name="password" class="inputtext" maxlength="24">
					...
					<p>
						...
						<input type="password" value="<?php echo set_value('passconf') ?>" id="passconf" name="passconf" class="inputtext" maxlength="24">

					</p>
					<?php
						$is_checked_s = "checked='checked'";
						if($_POST){
							$is_checked_s = "";
					...
					?>
					<p>
						<input type="checkbox" value="1" name="subscribe" id="subscribe" <?php echo $is_checked_s; ?>>
						
						<label for="subscribe">Receive special discounts and sneak previews via email.</label>
					</p>
		...
		<div class="accountinputbox">
			...
			<fieldset id="billing_information">
				...
				<p>
					...
					<label for="bill_state">State*<?php echo form_error('bill_state', '<span class="error">&nbsp;&nbsp;', '</span>'); ?>
			...
			<fieldset id="shipping_information" <?php echo $style?>>
				...
				<p style="width:60px;float:left;">
					...
					<input type="text" value="<?php echo $this->session->userdata('ship_zipcode') ?>" readonly="true" id="ship_zipcode" name="ship_zipcode" class="inputtext" style="width:60px;" maxlength="12" onBlur="javascript:zipcodes(this.value);" >
