<?php
	if (count($this->special_page_with_product) >= 1) {
?>
<div id="workswellwith">
	<h4>Featured Products</h4>
	<?php
	foreach ($this->special_page_with_product as $works) {
		$query = $this->db->query(
			...
			"FROM product as p WHERE id = " . $works->pid
		);
		...
		<div class="info">
			...
			<div style="width:50px;height:25px;float:left;overflow:hidden;">
				<!-- <fb:like action='like' colorscheme='light' 
						expr:href='data:/shop/<?php echo strtolower($works_well_with[0]->catalog_name) ?>/<?php echo strtolower($works_well_with[0]->url) ?>'
