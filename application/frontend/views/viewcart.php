...
<body>
<div id="header">
  <div id="logo"> <a href="/">Josie Maran, Luxury with a Conscience</a> </div>
...
<div id="topnav" style="font-size:15px;">
  <!--
			...
			-->
  <b>View Cart</b> &raquo; Checkout &raquo; Confirm Order  &raquo; Thank You </div>
<div id="pagetitle">
  <h1>
    <!--View Cart-->
  </h1>
...
$error_messages_zipcode = $this->session->userdata('error_messages_zipcode');


if (isset($error_messages_zipcode) && $error_messages_zipcode == 'yes') {



    ?>
<div class="errormessage">
  <p class="error">Update the state and zip code fields to match the shipping information you provided</p>
</div>
<?php

 $this->session->unset_userdata('error_messages_zipcode');	
...
<table border="0" cellspacing="5" cellpadding="5" width="100%" style="font-size:1em;margin-bottom:0;">
  ...
  <tr>
    <td colspan="4"><div style="width:100%;height:1px;margin-bottom:10px;">&nbsp;</div></td>
  </tr>
  <tr>
  	<td colspan="4" id="cart_listing" >
...
foreach ($this->cart->contents() as $items):
 ...
 ?>
  ...
  <tr>
    <td>&nbsp;</td>
    <td colspan="3"><div style="width:100%;height:1px;border-bottom:1px solid #F3E4E9;margin-bottom:15px;margin-top:15px;"> &nbsp;</div></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" class="cartvalue"><?php $cart_country = $this->session->userdata('cart_country'); ?>
      <select name="country" id="country" size="1" class="dropdown" onChange="get_shipping_option(this.value, '<?=site_url()?>ajax/get_shipping_state', 'shipping_state_div', 'yes')" >
        <option



                value="1" <?php echo isset($cart_country) && $cart_country == 1 ? 'selected="selected"' : '';?> > US </option>
        <option



                value="40" <?php echo isset($cart_country) && $cart_country == 40 ? 'selected="selected"' : '';?> > Canada </option>
      </select>
    </td>
  </tr>
  <tr>
    <td class="cartvar"> Shipping Destination State
      <?php 
		   ...
		   if (isset($this->ErrorMessage) && (!empty($this->ErrorMessage))) {
			...
			if($this->input->post('state') == '-1'){
				...
				echo '<br><span class="error" style="color:#D02121" >&nbsp;&nbsp;required</span>';
    ...
    <td colspan="3" class="cartvalue" id="shipping_state_div"><select name="state" id="state" size="1" class="dropdown" onChange="get_shipping_option(this.value, '<?=site_url()?>ajax/get_shipping_option', 'shipping_option_div', 'no')"  >
...
?>
  <input type="hidden" name="zipcodeInput" value="-1">
  <?php }else {

?>
  <tr>
    <td class="cartvar"> Shipping Zipcode
      <?php 
    ...
    <td colspan="3" class="cartvalue"><input type="text" name="zipcodeInput" value="<?php if($this->session->userdata('ship_zipcode') != '-1') { echo $this->session->userdata('ship_zipcode'); } ?>" id="zipcodeInput" class="inputtext" onBlur="javascript:update_shipping_option();" >
    </td>
    </td>
  </tr>
  <?php } ?>
  <tr>
    <td class="cartvar"> Shipping Option
      <?php 
    ...
    <td colspan="3" class="cartvalue" id="shipping_option_div"><select name="shipping_method" id="shipping_method" size="1" class="dropdown" onChange="update_shipping_option()">
        <option value='-1'>Please select</option>
        <?php
...
if ($IS_FREE_SHIPPING2 == TRUE) {
 ...
 ?>
  <tr id="free_shipping_row" style=" <?=$shipping_style?>">
    <td>&nbsp;</td>
    <td colspan="3" style="text-align:left; color: green" id="free_shipping"> -&#36;<?php echo $shipping_price; ?> Free Shipping </td>
  </tr>
  <tr>
    <td class="cartvar">Is it a PO BOX?</td>
    <td colspan="3" class="cartvalue"><input type="checkbox" name="pobox" value="pobox"



               id="pobox" <?php if($this->session->userdata('pobox') == 'pobox') { echo 'checked="checked"'; }  ?> >
      &nbsp;
      <label
  ...
  <tr>
    <td class="cartvar">Estimated Tax</td>
    <td colspan="3" class="cartvalue" id="productTax">$<?php echo $this->ProductTax; ?></td>
  </tr>
  <?php if ($this->session->userdata('VoucherCode') && isset($this->ShoppingCart->voucher_sub_total)) { ?>
  <tr>
    <td class="cartvar">Enter eGift Card Code</td>
    <td colspan="3" style="text-align:left; color:green" id="vouchercodeInput_td" > Code: "<?php echo $VoucherCode[0]->code ?>"<br>
      Amount: -$<?php echo number_format($this->ShoppingCart->voucher_sub_total, 2) ?>&nbsp;&nbsp;
      <input type="hidden" name="vouchercodeInput" value="<?php echo $this->session->userdata('cart_voucher'); ?>"
  ...
  <tr>
    <td class="cartvar">Enter eGift Card Code</td>
    <td colspan="3" class="cartvalue" id="vouchercodeInput_td" ><input type="text" name="vouchercodeInput" value="<?php echo set_value('vouchercodeInput') ?>" id="vouchercodeInput" class="inputtext" onBlur="javascript:update_shipping_option();" >
    </td>
  </tr>
  <?php
  ...
  <tr>
    ...
    <td colspan="3" style="text-align:left; color:green" id="discountcodeInput_td" ><?php if ($this->Promo_FreeShipping == 1): ?>
      <?php
      ...
      <?php if ($this->session->set_userdata('cart_shipping_method') <= 1) { ?>
      ...
      <?php }?>
      <?php endif;?>
      <input type="hidden" name="discountcodeInput" value="<?php echo $this->session->userdata('cart_promo'); ?>" id="discountcodeInput" class="inputtext">
  ...
  <tr>
    <td class="cartvar">Enter Promo Code</td>
    <td colspan="3" class="cartvalue" id="discountcodeInput_td" ><input type="text" name="discountcodeInput" value="<?php echo set_value('discountcodeInput') ?>"



               id="discountcodeInput" class="inputtext" onBlur="javascript:update_shipping_option();" >
    </td>
  </tr>
  <?php 
  ...
  <tr>
    ...
    <td colspan="3" class="cartvalue" id="grand_total"> $<?php echo $this->Sum ?> </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td colspan="3" class="cartvalue"><?php #echo form_submit('', 'Update your Cart'); ?>
...
?>
  <tr>
    <td colspan="6"><br>
      <font size="2" color="black" align="centre"><?php echo $webpages->page_content; ?></font></td>
  </tr>
  <?php } ?>
</table>
