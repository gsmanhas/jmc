        ...
        <table width="98%" cellspacing="0" cellpadding="3" border="0">
            <tbody>
                ...
                <tr valign="top">
                    <td width="20%">
                        <?php
                            $item = $this->db->query("SELECT gift_voucher_image_small as image  FROM gift_voucher WHERE id=?", $this->item['id']);
                            $result = $item->row();
                        ?>
                        <?php if($result->image != ""): ?> <img src="<?php echo base_url() . $result->image?>" alt="" /> <?php endif;?>
                    </td>
                    <td colspan="4">
                        <p>Hi <?php echo $this->item['options']['To'] ?>,</p>

                        <p>You have received a Josie Maran Cosmetics eGift Card from <?php echo $this->item['options']['From'] ?>.</p>

                        <p><?php echo $this->item['options']['Message'] ?></p>
