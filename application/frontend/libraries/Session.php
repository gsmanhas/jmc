...
*/
   ...
   * 還原庫存數量
   */
  function _reduction_in_stock()
  {
     // print_r($_SESSION['cart_contents']);
	log_message('debug', 'Session 出現在這裡 _reduction_in_stock');
     if (isset($_SESSION['cart_contents'])) {
          foreach ($_SESSION['cart_contents'] as $key => $value) {
               if (is_array($value)) {
                    $this->object->db->trans_start();              
                    $sql = "UPDATE product as p SET in_stock = p.in_stock + ? WHERE id = ?";
                    $this->object->db->query($sql, array($value['qty'], $value['id']));
                    // echo $this->object->db->last_query();
                    $this->object->db->trans_complete();              
               }
          }
     }
  }
