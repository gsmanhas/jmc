...
* 
*/
class Guestcheckout extends MY_Controller
{
	...
	public function _remap()
	{
		...
		if ($this->session->userdata('user_id') != "") {
			//redirect("/membercheckout", 'refresh');
		}
	...
	public function index()
	{
	   ...
	   $bill_country = 1;
		...
		$query = $this->db->query("SELECT t.id, s.state,
                                    ...
                                    ORDER BY `sharthand` ASC", $bill_country);
		...
		if($shipping_option[0]['id'] != 99) {
			...
			if (11 == $ds[0]['id']) {
				...
				$zip_tax = $this->zipcode_model->zipcde_wise_tax($this->session->userdata('ship_zipcode'));			
				$zip_tax = explode("|||", $zip_tax);
				if($zip_tax[1] != 'no'){
					$this->zip_wise_tax = $zip_tax[1];
				}
			}
		}*/
	...
	public function email_check($email)
	{
		...
		if($email == $confirm_email){
			...
			$query = $this->db->query('SELECT * FROM users WHERE email = ? and is_delete = 0', $email );
			$u = $query->result();
			...
			if (count($u) >= 1) {
					...
					if ($this->encrypt->decode($u[0]->password)== $this->input->post('password')) {
						...
						$this->ErrorMessage1 = 'yes'; 
						$this->session->set_flashdata('wrong_password', 'yes');				
	...
	public function user_need_to_added($email)
	{
		...
		if($email == $confirm_email){
			...
			if (count($u) < 1) {
				...
				$ppassword = strlen($this->input->post('password'));
				...
				$User = array(
					...
					'evening_phone'     => $this->input->post('phone'),
					'skin_color_id'     => '',
					'year_of_birth'     => '',
					'month_of_birth'    => '',
					'day_of_birth'      => '',
					...
					'bill_evening_phone'=> $this->input->post('phone'),
					...
					'ship_evening_phone'=> $this->input->post('phone'),
	...
	private function _submit_validate() {
		...
		if($this->input->post('password') or $this->input->post('passconf')){
		...
		if ($this->input->post('is_only_egift') != 'yes') {
			if ($this->input->post('shipSame') != 1) {
				$this->form_validation->set_rules('ship_first_name', 'Recipient First Name', 'required');			
				$this->form_validation->set_rules('ship_last_name', 'Recipient Last Name');			
				$this->form_validation->set_rules('ship_address2', 'Address2');			
	...
	private function _inventoryControl()
	{
		...
		foreach ($this->cart->contents() as $item) {
			if(key_exists("type", $item) && $item['type'] == 'voucher') continue;
	...
	private function _confirm()
	{
	  ...
	  if ($this->session->userdata('Amount') > 0) {
			...
			if ($this->_submit_validate() === FALSE) {
			   ...
			   $is_price_zero = $this->session->userdata('is_price_zero');
				if($is_price_zero == 'yes'){
					$this->error_messages_amount = 'yes';
				}
		...
		$this->user_need_to_added($this->input->post('email'));	
		
		$this->ShoppingCart = new SystemCart();
		
		if ($this->input->post('shipSame') == 1) { 
		 
		     $ship_zipcode_v = $this->session->userdata('ship_zipcode_2');
			
			if($ship_zipcode_v!= $this->input->post('bill_zipcode')) {
				$this->session->set_userdata('error_messages_zipcode', 'yes');	
				redirect('viewcart');
			}
		
			$this->ShoppingCart->DestinationState($this->input->post('bill_state'));
			$this->session->set_userdata('ship_zipcode', $this->input->post('bill_zipcode'));
		}else {
			$this->ShoppingCart->DestinationState($this->session->userdata('cart_state'));
			$this->session->set_userdata('ship_zipcode', $this->input->post('ship_zipcode'));
		}	
		...
		$this->form_url_s = 'guestcheckout/submit';
		$this->load->view('checkout_confirm');
	}
	...
	private function _submit()
	{
		...
		if ($this->input->post('go_func') == 'confirm') {
			$this->_confirm();
			return;
		}
		
		if ($this->input->post('go_func') == 'confirm_edit') {
			$this->index();
			return;
		}
	...
	public function Using_Authorize_net()
	{
		...
		if (is_array($DiscountCode)) {
            ...
            //echo $DiscountCode[0]->discount_type;exit;
