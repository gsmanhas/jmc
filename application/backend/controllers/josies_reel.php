...
* 
*/
class Josies_reel extends CI_Controller
{
	...
	public function index()
	{
		...
		$result = $this->db->query("SELECT * FROM josies_reel WHERE is_delete = 0 ORDER BY ordering asc");
		$this->josies_reel = $result->result();
			
		$this->load->view('josies_reel');
	}
	
	public function addnew()
	{
		$this->load->view('josies_reel_addnew');
	}
	...
	{		
		...
		$this->db->trans_start();
		$this->db->insert('josies_reel', $josies_reel);
		$this->db->trans_complete();
		
		if ($this->db->trans_status() === FALSE) {
			$this->db->trans_rollback();
			log_message('error', 'josies_reel->save() 交易錯誤');
		}
		
		redirect('josies_reel/success', 'refresh');
	}
	...
	public function _update_save()
	{
		...
		$this->db->update('josies_reel', $josies_reel);
		$this->db->trans_complete();
		
		if ($this->db->trans_status() === FALSE) {
			$this->db->trans_rollback();
			log_message('error', 'josies_reel->_update_save() 交易錯誤');
		}
	...
	{
		$this->message = "Josies Reel Saved Successfully";
		$this->index();
	}
	...
	public function _remove($ndx)
	{
		...
		if (!empty($ndx)) {
			...
			if (is_array($ids) && (count($ids) >= 1)) {
				$this->db->trans_start();
				foreach ($ids as $id) {
					$this->db->query('update josies_reel set is_delete = 1, updated_at = \'' . 
	...
	public function _save_order()
	{
		...
		if (count($_POST['lists']) != count($_POST['order'])) {
			...
			foreach ($lists as $list) {
				...
				$this->db->update('josies_reel', $Ordering);
	...
	{		
		...
		$this->db->update('josies_reel', $Publish);
		$numrows = $this->db->affected_rows();
		$this->message =  $numrows . " records updated";
	...
	{		
		$result = $this->db->query("SELECT * FROM josies_reel WHERE is_delete = 0 and id = " . $ndx);
		$this->josies_reel = $result->result();
		$this->load->view('josies_reel_update');
	}
