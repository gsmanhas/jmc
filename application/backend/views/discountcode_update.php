	...
	<div id="page-wrapper" class="fluid">
		<div id="main-wrapper">
			<div id="main-content">				
				...
				<div class="hastable">	
					<form id="frmMain" name="frmMain" method="post" action="<?php echo base_url() ?>admin.php/discountcode">
						...
						<table border="0" cellspacing="5" cellpadding="5">
							<tr>
								<td style="width:50%">
									<table border="0" cellspacing="5" cellpadding="5">
										...
										<tr id="discount_fixex_amount">
											...
											<td><input type="text" id="discount_percentage" name="discount_percentage" class="field text small" value="<?php echo $this->discountcode[0]->discount_percentage ?>" maxlength="255" tabindex="2" style="width:100%"></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td><input type="text" name="xuses" class="field text small" value="<?php echo $this->discountcode[0]->xuses ?>" maxlength="255" tabindex="2" style="width:100%"></td>
										</tr>
										<tr>
											...
											<td>
												<?php 
													$checked = "";
													if ($this->discountcode[0]->can_free_shipping == 1) {
										...
										<tr>
											...
											<td>
												<?php
													$checked = "";
													if ($this->discountcode[0]->apply_ones == 1) {
														$checked = "checked=\"checked\"";
													}
								...
								<td style="vertical-align:top;">
									<table border="0" cellspacing="5" cellpadding="5">
                                        <tr class="free_gift" <?php echo $this->discountcode[0]->discount_type == 5 ? 'style="display:table-row"' : ''?>>
                                            <td>
										...
										<tr>
											<td class="fieldlabel" style="text-align:left;">
												<?php
												$checked = '';
												// $q1 = $this->db->query("SELECT cid FROM discountcode_rel_catalogs WHERE d_id = " . $this->discountcode[0]->id);
												$q2 = $this->db->query("SELECT pid FROM discountcode_rel_products WHERE d_id = " . $this->discountcode[0]->id);
												// $discountcode_rel_catalogs_count = $q1->result();
												$discountcode_rel_products_count = $q2->result();
												// if (count($discountcode_rel_catalogs_count) >= 1 || count($discountcode_rel_products_count) >= 1) {
												// 	$checked = "checked=\"checked\"";
												// 	$color = "#ffffff";
												// 	$disabled = "";	
												// } else {
												...
												if (count($discountcode_rel_products_count) >= 1) {
													$checked = "checked=\"checked\"";
													$color = "#ffffff";
													$disabled = "";
												} else {
										...
										<tr>
											<td>
												...
												<select name="product_list[]" id="product_list" multiple size="29" style="border:1px solid #ccc;width:100%; background:<?php echo $color; ?>" <?php echo $disabled ?>>
													...
													$queryB = $this->db->query("SELECT * FROM discountcode_rel_products WHERE d_id = " . $this->discountcode[0]->id);
													$Product_Ids = $queryB->result();
													
													// $Cids = explode(',', $Catalog_Ids);
													...
													foreach ($products as $product) {
														...
														foreach ($Product_Ids as $Pids) {
															if ($Pids->pid == $product->id) {
																// if ($Pids->cid == $Catalog->id) {
																	$is_selected02 = "selected=\"selected\"";
																// }
															}
														}
														printf("<option style=\"margin-bottom:2px;\" value=\"%s\" %s>%s</option>", $product->id, $is_selected02, $product->name);
													}
