...
class browser extends uploader
{
    ...
    public function queue_resize() {
        $rs = mysql_connect($this->config['db']['host'], $this->config['db']['user'], $this->config['db']['password']);
        mysql_select_db($this->config['db']['database'], $rs) || die ("Can't select Database");
        mysql_query('SET NAMES UTF8;', $rs);

        $result = mysql_query("SELECT * FROM image_resize_queue");
        while ($obj = mysql_fetch_object($result)) {
            $size = filesize($this->typeDir . '/../.temp/' . $obj->name);
            if ($size > 1048576) {
                $resize = true;
            } else {
                $resize = false;
            ...
            $this->moveUploadFile(array(
                    'name' => $obj->name,
                    'tmp_name' => $this->typeDir . '/../.temp/' . $obj->name,
                    'error' => null
                ), $obj->dir, true, $resize);

            mysql_query("DELETE FROM image_resize_queue WHERE id={$obj->id}", $rs);
        }
            ...
            foreach ($this->file['name'] as $i => $name) {
                if ($i < 5) {
                    ...
                    $this->moveUploadFile(array(
                        ...
                        'tmp_name' => $this->file['tmp_name'][$i],
                        'error' => $this->file['error'][$i]
                    ), $this->typeDir . '/../.temp', false, false);

                    mysql_query("INSERT INTO `image_resize_queue`(`name`, `dir`)
                                    VALUES ('{$name}','{$dir}');", $rs);
                }
            }
            return implode("\n", $return);
    ...
    protected function moveUploadFile($file, $dir, $makeThumb = true, $resize = true)
    {
        ...
        if($resize && in_array(file::getExtension(basename($target)), array('jpg', 'png', 'gif', 'bmp')))
            $this->smart_resize_image($target, 1024, 0, true);
